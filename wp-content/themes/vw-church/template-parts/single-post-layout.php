...
<article id="post-<?php the_ID(); ?>" <?php post_class('inner-service'); ?>>
    ...
    <?php } ?>
    <?php if( get_theme_mod( 'vw_church_single_post_category',true) == 1) { ?>
        <div class="single-post-category mt-3">
            <span class="category"><?php esc_html_e('Categories:','vw-church'); ?></span>
            <?php the_category(); ?>
        </div> 
    <?php }?>
    ...
    <?php if(get_theme_mod('vw_church_toggle_tags',true)==1){ ?>
    ...
    <?php } ?>
        ...
        if ( is_singular( 'attachment' ) ) {
            ...
            the_post_navigation( array(
                'prev_text' => _x( '<span class="meta-nav">Published in</span><span class="post-title">%title</span>', 'Parent post link', 'vw-church' ),
            ) );
        } elseif ( is_singular( 'post' ) ) {
            // Previous/next post navigation.
            the_post_navigation( array(
                'next_text' => '<span class="meta-nav" aria-hidden="true">' . __( 'Next', 'vw-church' ) . '</span> ' .
                    '<span class="screen-reader-text">' . __( 'Next post:', 'vw-church' ) . '</span> ' .
                    '<span class="post-title">%title</span>',
                'prev_text' => '<span class="meta-nav" aria-hidden="true">' . __( 'Previous', 'vw-church' ) . '</span> ' .
                    '<span class="screen-reader-text">' . __( 'Previous post:', 'vw-church' ) . '</span> ' .
                    '<span class="post-title">%title</span>',
            ) );
        }
    ?>
